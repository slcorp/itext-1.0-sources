...
import static com.lowagie.text.pdf.AsianFontMapper.*;
...
public class PdfGraphics2D extends Graphics2D {
    ...
    /**
     * SL does not use FRACTIONAL METRICS for TEXT layout / positioning.
     */
    private final static Object SL_DEFAULT_FRACTIONALMETRICS_HINT =
			RenderingHints.VALUE_FRACTIONALMETRICS_OFF;

    private PdfGraphics2D() {
        // dg2.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_ON);
        // setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_ON);
        dg2.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, SL_DEFAULT_FRACTIONALMETRICS_HINT);
        setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, SL_DEFAULT_FRACTIONALMETRICS_HINT);
        dg2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_DEFAULT);
        dg2.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_DEFAULT);
	...
	// DEVNOTE - SL Mod to display CJK characters when in that locale
	this.fontMapper = makeFontMapperInstance();
    }

/**
 * Make the correct type of font mapper based on the locale.
 * Use the appropriate font defined in the iTextAsian.jar file
 * which maps to fonts in the adobe asian fonts pack for adobe reader.
 *
 * @return
 */

private FontMapper makeFontMapperInstance ()
{
			// Use default font mapper - by default
    ...
    PdfGraphics2D(PdfContentByte cb, float width, float height, FontMapper fontMapper, boolean onlyShapes, boolean convertImagesToJPEG, float quality) {
        ...
        if (!onlyShapes) {
            ...
            if (this.fontMapper == null) {
				...
				// DEVNOTE - SL Mod to display CJK characters
				// when in that locale
		this.fontMapper = makeFontMapperInstance();
	    }
        }
...
//        setPaint(paint, false, 0, 0);
    ...
    public void setRenderingHint(Key arg0, Object arg1) {
		...
		// SL DEVNOTE - check for null arg1
		if (arg1 != null) { 
			rhints.put(arg0, arg1); 
		} 
		else { 
    ...
    public void drawLine(int x1, int y1, int x2, int y2) {
				...
				// Is this a zero length line?
	if ((x1 == x2) && (y1 == y2)) {
				// Yes, zero length line
				// Draw fillled rectangle for 1 pixel dot
		fill(new Rectangle2D.Double(x1 -0.5, y1 -0.5, 1, 1));
		return;
	}
