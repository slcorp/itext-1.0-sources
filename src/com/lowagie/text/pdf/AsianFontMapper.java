...
public class AsianFontMapper extends DefaultFontMapper {
	...
	public BaseFont awtToPdf(Font font) {
		try {
			...
			String styleModifier = getStyleModifier(font);

			if (p != null){
				return BaseFont.createFont(p.fontName + styleModifier, p.encoding, p.embedded, p.cached, p.ttfAfm, p.pfb);
			}else{
				return BaseFont.createFont(defaultFont + styleModifier, encoding, true);
			}
		}
		catch (Exception e) {
			e.printStackTrace();
...
/**
 * Get the style modifer for the font
 *
 * @param font
 * @return
 */
private String getStyleModifier (Font font)
{
